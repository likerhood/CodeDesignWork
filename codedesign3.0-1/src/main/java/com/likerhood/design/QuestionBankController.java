...
public class QuestionBankController {
    ...
    public String createPaper(String candidate, String number){
        ...
        // 遍历选择题列表
        for (int id = 0; id < choiceQuestionList.size(); id++) {
            detail.append("第").append(id + 1).append("题：").append(choiceQuestionList.get(id).getName()).append("\r\n");
            Map<String, String> option = choiceQuestionList.get(id).getOption();
            for (String key : option.keySet()) {
                detail.append(key).append("： ").append(option.get(key)).append("\r\n");
            }
            detail.append("答案： ").append(choiceQuestionList.get(id).getKey()).append("\r\n");
        }
